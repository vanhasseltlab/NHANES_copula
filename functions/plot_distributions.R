# helper functions for visualizing the pairwise distribution of observation and simulation data sets
...
plot_comparison_distribution_sim_obs_generic <- function(sim_data, obs_data, variables = NULL, 
  ...
  for (i in 1:nrow(combination_variables)) {
    ...
    if (plot_type != "density") {
      # point_plots[[combination]] <- ggplot(mapping = aes(x = combination_variables[i, 1], y = combination_variables[i, 2])) +
    ...
    if (plot_type != "points") {
      # density_plots[[combination]] <- ggplot(mapping = aes(x = combination_variables[i, 1], y = combination_variables[i, 2])) +
  ...
  for (i in variables_str) {
    univariate_plots[[i]] <- total_data %>% 
      ggplot(aes(y = i, x = "type", fill = "type")) +
